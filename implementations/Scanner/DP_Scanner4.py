...
def handle_msg(msgs: List[WebSocketMessage]):
    ...
    for m in msgs:
        ...
        if (m.event_type == 'A'):  # per sec msgs
            ...
            merged_df = top_symbols.groupby('symbol', as_index=False).agg(
                {'symbol': 'first', 'open': 'first', 'high': 'first', 'low': 'first', 'close': 'first', 'vwap': 'first', 'volume': 'sum',
            ...
            #print(Get_Real_Time(merged_df.iloc[-1]['symbol']) )
    ...
    row_index = 0
    symbol_value = merged_df.loc[row_index, 'symbol']
    print('Symbol Value is:  ',symbol_value)
    answeris = Get_Real_Time(symbol_value)

...
def handle_msg(msgs: List[WebSocketMessage]):
    ...
    df2 = pd.DataFrame(aggs,
                      columns=['symbol', 'open', 'high', 'low', 'close', 'volume', 'average_size', 'end_timestamp', 'otc', 'MF'])

    for m in msgs:
        #print(m)
        df_columns = ['symbol', 'open', 'high', 'low', 'close', 'volume', 'average_size', 'end_timestamp', 'otc']
        ...
        if (m.event_type == 'AM'):  # per sec msgs
            while len(df) <= 100:
                ...
                df = pd.DataFrame(aggs,
                        columns=['symbol', 'open', 'high', 'low', 'close', 'volume', 'average_size', 'end_timestamp', 'otc'])
            ...
            merged_df = top_symbols.groupby('symbol', as_index=False).agg(
                {'open': 'first', 'high': 'first', 'low': 'first', 'close': 'first', 'volume': 'sum',
            ...
            #at this point we have a df of max money flow symbols trading, next get indicators
            print('I am now waiting around ...')

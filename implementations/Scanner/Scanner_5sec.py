...
class MarketMonitor:
    ...
    across different symbols using Polygon.io's WebSocket API.
    """
    ...
    def __init__(self, api_key: str):
        ...
        # 5-second snapshot data
        self.trade_counts_5s: Dict[str, int] = {}
        self.cash_volume_5s: Dict[str, float] = {}
        self.cash_traded_5s = 0.0
    ...
    def _process_trade(self, trade: EquityTrade):
        ...
        # Update trade counts
        self.trade_counts_5s[trade.symbol] = self.trade_counts_5s.get(trade.symbol, 0) + 1
        ...
        if isinstance(trade.price, float) and isinstance(trade.size, int):
            ...
            # Update 5-second statistics
            self.cash_traded_5s += cash_value
            self.cash_volume_5s[trade.symbol] = self.cash_volume_5s.get(trade.symbol, 0) + cash_value
    ...
    def _display_statistics(self):
        ...
        # Display 5-second snapshot
        self._print_centered("--- Past 5 seconds ---")
        self._print_centered(f"Tickers seen (5s): {len(self.trade_counts_5s)}")
        self._print_centered(f"Trades seen (5s): {sum(self.trade_counts_5s.values())}")
        self._print_centered(f"Cash traded (5s): {self.cash_traded_5s:,.0f}")
        ...
        sorted_symbols = sorted(self.trade_counts_5s.items(), key=lambda x: x[1], reverse=True)[:10]
        for symbol, trades in sorted_symbols:
            self._print_centered(
                "{:<15}{:<20}{:<20,.0f}{:<20}{:<20,.0f}".format(
                    symbol,
                    trades,
                    self.cash_volume_5s.get(symbol, 0),
    ...
    def _run_display_loop(self):
        ...
        while True:
            ...
            # Reset 5-second counters
            self.trade_counts_5s.clear()
            self.cash_volume_5s.clear()
            self.cash_traded_5s = 0
            time.sleep(5)

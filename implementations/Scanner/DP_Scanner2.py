...
def handle_msg(msgs: List[WebSocketMessage]):
    ...
    for m in msgs:
        ...
        if (m.event_type == 'AM'):  # per sec msgs
            ...
            sys.stdout.flush()
            #print(merged_df.symbol, merged_df.close, merged_df.volume, merged_df.average_size )
            #columns_to_display = ['symbol', 'close', 'volume', 'average_size']
            #print(merged_df[columns_to_display])

            # at this point we have a df of max money flow symbols trading, next get indicators
            #print('I am now waiting around picking myass... myass=1 ')

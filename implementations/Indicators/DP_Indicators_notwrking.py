...
#from DP7 import stock_data_1D
#from DP7 import Get_Historical_Day

from globals import Ticker
...
#********************************
def Expand_1minute_Data(df, dfD1):
    ...
    # resample and set up 1 Day (Current Day) bars
    dfD = df.resample("1440min").agg({
        "open": "first", "high": "max", "low": "min", "close": "last", "vwap": "last",
        "volume": "sum", "timestamp": "last"})
    # print(dfD)

    dfD['SMA3_1D'] = ta.sma(dfD['close'], length=3)
    dfD['Sl_SMA3'] = ta.slope(ta.sma(dfD['close'], length=3), length=1)
    dfD['SMA14'] = ta.sma(dfD['close'], length=14)
    dfD['Sl_SMA14'] = ta.slope(ta.sma(dfD['close'], length=14), length=1)
    dfD['SMA50'] = ta.sma(dfD['close'], length=50)
    dfD['Sl_SMA50'] = ta.slope(ta.sma(dfD['close'], length=50), length=1)
    dfD['SMA200'] = ta.sma(dfD['close'], length=200)
    dfD['Sl_SMA200'] = ta.slope(ta.sma(dfD['close'], length=200), length=1)
    # print(dfD)

    bbD = ta.bbands(dfD['close'], length=14, std=2)
    dfD = pd.concat([dfD, bb15], axis=1)
    dfD = dfD.rename(columns={'BBL_14_2.0': 'BBL_14_2'})
    dfD = dfD.rename(columns={'BBM_14_2.0': 'BBM_14_2'})
    dfD = dfD.rename(columns={'BBU_14_2.0': 'BBU_14_2'})
    dfD = dfD.rename(columns={'BBB_14_2.0': 'BBB_14_2'})
    dfD = dfD.rename(columns={'BBP_14_2.0': 'BBP_14_2'})

    desired_columns = ['open', 'high', 'low', 'close', 'vwap', 'volume', 'timestamp', 'SMA3_1D']
    clipped_df = dfD.loc[:, desired_columns]
    clipped_df.dropna(inplace=True)
    dfD = clipped_df

    dfD_today = dfD.iloc[[-1]]
    #print(dfD_today)

    from DP7 import stock_data_1D
    dfDH = stock_data_1D.df
    dfDH.dropna(inplace=True)
    #print(dfDH)


    dfDout = pd.concat( [dfDH, dfD_today], ignore_index=True)
    dfDout = pd.concat([dfDH, dfD_today])
    #print(dfDout)

    return df, df5, df15, dfDout

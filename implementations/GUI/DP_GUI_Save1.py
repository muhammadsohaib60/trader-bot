...
def custom_method(arg1, arg2):
    ...
    print(f"Custom method called with arguments: {arg1}, {arg2}")
...
def Chart_Plot(Ticker, df, df5, df15, dfD):
    ...
    fig.add_trace(go.Candlestick(x=df.index, open=df.open_1M, high=df.high_1M, low=df.low_1M, close=df.close_1M ), row=1, col=5)
    ...
    fig.add_trace(go.Bar(x=df.index, y=df.volume_1M, marker=dict(color='#7cadf8')), row=2, col=5)


    # Add 5M Candlestick Chart
    fig.add_trace(go.Candlestick(x=df5.index, open=df5.open_5M, high=df5.high_5M, low=df5.low_5M,close=df5.close_5M ), row=1, col=4)
    ...
    # Add Volume Chart to Row 2 of subplot
    fig.add_trace(go.Bar(x=df5.index, y=df5.volume_5M, marker=dict(color='#7cadf8')), row=2, col=4)

    # Add 15M Candlestick Chart
    fig.add_trace(go.Candlestick(x=df15.index, open=df15.open_15M, high=df15.high_15M, low=df15.low_15M, close=df15.close_15M ), row=1, col=3)
    ...
    # Add Volume Chart to Row 2 of subplot
    fig.add_trace(go.Bar(x=df15.index, y=df15.volume_15M, marker=dict(color='#7cadf8')), row=2, col=3)

    # Add Day Candlestick Chart
    fig.add_trace(go.Candlestick(x=dfD.index, open=dfD.open_D, high=dfD.high_D, low=dfD.low_D, close=dfD.close_D),row=1, col=2)
    ...
    # Add Volume Chart to Row 2 of subplot
    fig.add_trace(go.Bar(x=dfD.index, y=dfD.volume_D, marker=dict(color='#7cadf8')), row=2, col=2)
    ...
    Dyn_Ymin = df15.tail(1).low_15M - 1
    Dyn_Ymax = df15.tail(1).high_15M + 1
    #print(Dyn_Ymin.values[0])
    Dyn_Ymin = math.ceil(Dyn_Ymin.values[0] * 0.98)             # down 2%
    Dyn_Ymax = math.floor(Dyn_Ymax.values[0] * 1.02)            # up 2%

...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        ...
        value=DEFAULT_TICKER  # Set default ticker here
    ),
    ...
    dcc.Dropdown(
        ...
        value=DEFAULT_TIMEFRAME  # Set default timeframe here
    ),
...
@app.callback(
    ...
    [Input("ticker-dropdown", "value"), Input("custom-ticker-input", "value"), Input("timeframe-dropdown", "value")]
)
def update_chart(selected_ticker, custom_ticker, timeframe):
    global polygon_client

    ticker = custom_ticker.upper() if selected_ticker == "custom" and custom_ticker else selected_ticker.upper() if selected_ticker != "custom" else DEFAULT_TICKER # Use default ticker if no ticker provided

    try:
        df = fetch_stock_data(ticker, timeframe, polygon_client)

        if df.empty:
            return go.Figure(data=[go.Scatter(x=[], y=[])], layout=go.Layout(title=f"No data for {ticker} ({timeframe})")), f"No data found for {ticker} with the selected timeframe."

        df = calculate_indicators(df)

        if df.empty:
            return go.Figure(data=[go.Scatter(x=[], y=[])], layout=go.Layout(title=f"No data for {ticker} ({timeframe})")), f"Error calculating indicators for {ticker}."
        ...
        return fig, ""  # Return a tuple (figure, message)

    except Exception as e:
        print(f"General Error: {e}")
        return go.Figure(), f"An error occurred: {e}"  # Return a tuple
